...
func TestParse(t *testing.T) {
	...
	// odd number of chars
	if x, err := Parse("0"); x != nil || err != eidiom.ErrorParseInvalidFormat {
		t.Error(x, ToHexString(x), err)
	}
	if x, err := Parse("0ab"); x != nil || err != eidiom.ErrorParseInvalidFormat {
		t.Error(x, ToHexString(x), err)
	}

	// invalid char
	if x, err := Parse("0-ab"); x != nil || err != eidiom.ErrorParseInvalidFormat {
		t.Error(x, ToHexString(x), err)
	}
}
